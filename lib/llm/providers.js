/**
 * Resolve an OpenAI-compatible provider config for a modality, with a shared
 * fallback. Mirrors the sidecar's old _env() "first non-empty wins" behaviour.
 * @param {'VISION'|'STT'|'PDF_OCR'} modality
 * @returns {{apiKey:string|undefined, baseUrl:string|undefined, model:string|undefined}}
 */
export function resolveProvider(modality) {
  const pick = (...names) => {
    for (const n of names) { const v = process.env[n]; if (v) return v; }
    return undefined;
  };
  return {
    apiKey:  pick(`PULLMD_${modality}_API_KEY`,  'PULLMD_LLM_API_KEY'),
    baseUrl: pick(`PULLMD_${modality}_BASE_URL`, 'PULLMD_LLM_BASE_URL'),
...
export function abortController(opts = {}, ms) {
  ...
  return {
    signal: ctrl.signal,
    cleanup: () => { clearTimeout(timer); if (opts.signal) opts.signal.removeEventListener('abort', onAbort); },
  };
}
