...
export async function transcribeAudio(buffer, opts = {}) {
  ...
  if (mdl.startsWith('whisper')) form.append('response_format', 'verbose_json'); // adds duration
  ...
  try {
    ...
    if (data?.duration != null) out.audioSeconds = Math.round(Number(data.duration) * 10) / 10;
    return out;
  } finally { cleanup(); }
}
