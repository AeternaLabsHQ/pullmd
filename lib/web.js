...
import { matchRecipes, matchRecipesAgainst } from './recipes.js';
...
export async function extractWeb(url, options = {}) {
  ...
  const result = await convertWithReadability(url, body, comments, statusCode, rawFetchFn, effectiveExtractor);
  ...
  try {
    ...
    const rendered = await convertWithReadability(url, renderedHtml, comments, statusCode, rawFetchFn, effectiveExtractor);
