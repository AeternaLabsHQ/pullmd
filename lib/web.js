...
const MARKITDOWN_MEDIA_CONTENT_TYPES = new Set([
  'image/png', 'image/jpeg', 'image/gif', 'image/webp', 'image/bmp', 'image/tiff',
  'audio/mpeg', 'audio/mp3', 'audio/wav', 'audio/x-wav', 'audio/mp4',
  'audio/ogg', 'audio/flac', 'audio/aac', 'audio/webm',
]);
const MARKITDOWN_MEDIA_EXT_RE = /\.(png|jpe?g|gif|webp|bmp|tiff?|mp3|wav|m4a|ogg|flac|aac)$/i;
 ...
 *
 * Document types (MARKITDOWN_CONTENT_TYPES / MARKITDOWN_EXT_RE) always route.
 * Media types (image/audio) only route when MARKITDOWN_MEDIA env var is set.
 */
function isMarkitdownTarget(contentType, url) {
  const ct = (contentType || '').split(';')[0].trim().toLowerCase();
  const media = !!process.env.MARKITDOWN_MEDIA;

  if (MARKITDOWN_CONTENT_TYPES.has(ct)) return true;
  if (media && MARKITDOWN_MEDIA_CONTENT_TYPES.has(ct)) return true;

  if (ct === '' || ct === 'application/octet-stream') {
    let pathname;
    try { pathname = new URL(url).pathname; } catch { return false; }
    if (MARKITDOWN_EXT_RE.test(pathname)) return true;
    if (media && MARKITDOWN_MEDIA_EXT_RE.test(pathname)) return true;
  }
  return false;
}
...
export async function extractWeb(url, options = {}) {
  ...
  // Branches are mutually exclusive with decodeBody(), so arrayBuffer() is read once.
  if (!comments && isMarkitdownTarget(ct, url) && typeof res.arrayBuffer === 'function') {
    ...
    const converted = await markitdownClient(buf, { filename, contentType: ct, signal });
    if (!converted) {
    ...
    if (converted.usage) {
      metadata.llmModel = converted.usage.model || null;
      if (converted.usage.total_tokens !== undefined) metadata.llmTokens = converted.usage.total_tokens;
      if (converted.usage.prompt_tokens !== undefined) metadata.llmPromptTokens = converted.usage.prompt_tokens;
      if (converted.usage.completion_tokens !== undefined) metadata.llmCompletionTokens = converted.usage.completion_tokens;
    }
    if (converted.audioSeconds != null) metadata.audioSeconds = converted.audioSeconds;
    if (converted.imageSize) metadata.imageSize = converted.imageSize;
  ...
  if (!comments && !!process.env.MARKITDOWN_YOUTUBE && isYoutubeUrl(url)) {
    ...
    if (yt) {
      ...
      if (f.views) bits.push(`**Views:** ${f.views}`);
      if (f.published) bits.push(`**Published:** ${f.published}`);
      ...
      metadata.contentLength = trimmedMd.length;
      metadata.ytDuration = f.duration || null;   // surfaced into frontmatter by server.js
      metadata.ytViews = f.views || null;

      const markdown = formatHeader(title, url, null) + metaLine + trimmedMd;
...
export async function extractFile(buffer, options = {}) {
  const { filename, contentType, markitdownClient = convertViaMarkitdown } = options;
  const converted = await markitdownClient(buffer, { filename, contentType });
  if (!converted) {
    throw new Error('markitdown sidecar unavailable or could not convert this document');
  }
  const title = converted.title || filename || 'Document';
  const trimmedMd = converted.markdown.trim();
