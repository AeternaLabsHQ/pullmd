...
function isMarkitdownTarget(contentType, url) {
  ...
  if (ct === '' || ct === 'application/octet-stream') {
    try { return MARKITDOWN_EXT_RE.test(new URL(url).pathname); } catch { return false; }
  }
  return false;
}
...
export async function extractWeb(url, options = {}) {
  ...
  if (!comments && isMarkitdownTarget(ct, url) && typeof res.arrayBuffer === 'function') {
    ...
    const converted = await markitdownClient(buf, { filename: url, contentType: ct, signal });
    if (!converted) {
    ...
    const markdown = formatHeader(title, url, null) + converted.markdown.trim();
    const metadata = { ...emptyMetadata(), sourceUrl: url, statusCode, quality: qualityScore(converted.markdown) };
    metadata.contentLength = converted.markdown.trim().length;
