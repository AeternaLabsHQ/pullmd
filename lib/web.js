...
export async function extractWeb(url, options = {}) {
  ...
  if (!comments && typeof res.arrayBuffer === 'function') {
    let pathHint = '';
...
export async function extractFile(buffer, options = {}) {
  ...
  if (kind === 'image') {
    const media = await captionFn(buffer, { mimetype: contentType, filename });
    if (media) return fileResult(media, filename || 'Image', filename, 'image-caption');
  } else if (kind === 'audio') {
    const media = await transcribeFn(buffer, { mimetype: contentType, filename });
