 ...
 */
function decodeBytes(buffer, contentTypeHeader = '', _res) {
...
async function decodeBody(res, contentTypeHeader = '') {
  if (typeof res.arrayBuffer === 'function') {
    try {
      ...
      return decodeBytes(buffer, contentTypeHeader, res);
    } catch {
...
export async function extractWeb(url, options = {}) {
  ...
  if (!comments) {
    ...
    if (kind === 'image' || kind === 'audio') {
      const buf = bodyBytes ?? Buffer.alloc(0);
  ...
  const body = bodyBytes != null ? decodeBytes(bodyBytes, ct, res) : await decodeBody(res, ct);
