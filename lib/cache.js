...
/**
 * Read PULLMD_CACHE_RETENTION_DAYS: a non-negative integer number of days,
 * where 0 means unlimited (never prune). Unset or empty after trimming is the
 * normal "not configured" case (compose files pass ${VAR:-}) and falls back
 * silently. Any other malformed value warns once and falls back too, so a typo
 * never keeps the server from starting.
 */
export function readCacheRetentionDays(env = process.env, warn = console.warn) {
  ...
  warn(`PULLMD_CACHE_RETENTION_DAYS: invalid value "${raw}" (expected a non-negative integer number of days); using default ${DEFAULT_CACHE_RETENTION_DAYS}`);
...
export function createCache(dbPath = '/data/cache.db', { retentionDays = DEFAULT_CACHE_RETENTION_DAYS } = {}) {
  ...
  if (!Number.isInteger(retentionDays) || retentionDays < 0) {
    throw new TypeError(`createCache: retentionDays must be a non-negative integer (got ${typeof retentionDays} ${String(retentionDays)})`);
  }
